...
class Isimm extends MY_Controller {
    ...
    private function filterArray(&$array, $allowedKeys ){
		$result = [];
    ...
    private function getAllHypothesis(){
    ...
    private function addFormElem($formId, $elemId, $label, $placeholder = '', $required = false, $formType = 'number', array $formCol = ['col-sm-3', 'col-sm-7'] )
    {
	...
	private function getInvestmentDetailBySection($guid){

    	return $this->isimm_model->getInvestmentByGuid($guid);
	}


	private function getPlantDetail($plantGuid)
	{
		$userId = $this->session->userdata('id');
		return $this->isimm_model->getPlantByGuid($plantGuid, $userId);
	}

	private function  getSectionDetail_TOBE($sectionGuid){
	...
	private function getSectionDetail_ASIS($sectionGuid){
		return $this->isimm_model->getSectionByGuid_asis($sectionGuid);
	}

	private function splitTOBEParams($tobeParams, $allowed){
